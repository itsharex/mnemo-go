...
| 4 | pan189（天翼云盘） | 账密 + 验证码 | ✅ 已注册 | 包内单测 + mock/e2e | 个人云与家庭云容量独立；家庭云仅在右上角弹窗展示，真实账号待验收 |
| 5 | pan139（139 云盘） | 手机号/邮箱 + 密码 / Authorization | ✅ 已注册 | 包内配额单测 + mock/e2e | 根目录分个人云与家庭云；家庭云可浏览、下载、建目录、上传和重命名文件；家庭云容量仅在右上角弹窗展示，真实账号待验收 |
...
下载统一由 `dlengine` 使用 HTTP Range 分段续传。账号文件的签名链接更换后，只有文件标识和服务端 ETag/Last-Modified 均能确认内容未变时才复用已完成分片；缺少 Range 或无法确认身份时从头下载，避免拼接错误内容。直链任务重启后不保留敏感 URL/请求头，需重新添加。
...
|----------|:-------------:|:--------:|:--------:|:--------:|:----:|:--------:|:--------:|:--------:|
| pikpak | ✅ | ➖ | ✅(OSS PUT) | ➖（失败从头重试） | ✅(GCID) | ✅(refuse/skip/rename/overwrite) | ✅ | queue |
...
| ilanzou | ✅ | ✅(8MB) | ✅(≤8MB) | ✅（分片，>8MB） | ✅(MD5) | ➖ | ✅ | queue |
| onedrive | ✅ | ✅(10MB session) | ✅(≤4MB) | ✅ | ➖ | ❌(固定rename) | ✅ | queue |
| dropbox | ✅ | ✅(8MB session) | ✅(≤150MB) | ✅（会话，>150MB） | ➖ | ❌(固定add) | ✅ | queue |
| yike | ✅ | ✅(4MB) | ➖ | ➖ | ✅ | ➖ | ✅ | queue |
| guangya | ✅ | ✅(OSS multipart) | ➖ | ➖ | ✅ | ➖ | ✅ | queue |
| webdav | ✅ | ➖ | ✅(PUT) | ➖ | ➖ | ✅ | ✅ | direct |
| s3 | ✅ | ✅(16MiB multipart) | ✅(<16MiB PUT) | ✅（multipart/ListParts） | ➖ | ✅ | ✅ | direct |

> ✅ aliopen/pan123/pan189/onedrive 已持久化上传会话；Dropbox 也保存远端 session 与已确认偏移。
> ✅ webdav/s3 的冲突策略与进度回调已实现（ConflictPolicy + ProgressReader）；S3 16MiB 及以上上传自动使用 multipart，重名策略包含 `skip`。小文件 PUT 和通用 WebDAV PUT 不具备可移植的服务端断点协议，重试会从头上传。
...
| lanzou | ✅ | ➖ | ✅ | ✅ | ➖ | ➖ |
| ilanzou | ✅ | ✅ | ✅ | ✅ | ➖ | ➖ |
| onedrive | ✅ | ➖(与 Trash 同一接口) | ➖ | ➖ | ➖ | ➖ |
| dropbox | ✅ | ➖(与 Trash 同一接口) | ✅(仅文件) | ✅(仅文件) | ➖ | ➖ |
| yike | ✅(照片) | ➖ | ✅(照片) | ✅(照片) | ➖ | ➖ |
| guangya | ✅ | ➖ | ✅ | ✅ | ➖ | ➖ |
| webdav | ➖ | ✅ | ➖ | ➖ | ➖ | ➖ |
| s3 | ➖ | ✅ | ➖ | ➖ | ➖ | ➖ |
...
- 前端回归覆盖预览异步结果隔离、清晰度切换、播放进度、分享导入目标账号、同步列表状态及设置读取失败保护。视频、图片和音频预览已调整；PDF、DOCX、XLSX、PPTX 已提供只读预览，但不同复杂文档的视觉还原效果仍需真实文件验收；Markdown、文本预览的完整视觉重做尚未完成。
