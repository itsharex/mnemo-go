...
func TestConfigScopeSeparatesAccountsAndDirectories(t *testing.T) {
 ...
 cfg.UserID="two";if first==configScope(cfg){t.Fatal("account change reused snapshot scope")}
 cfg.UserID="one";cfg.RemoteDir="folder";if first==configScope(cfg){t.Fatal("directory change reused snapshot scope")}
}
...
func TestLocalSubsecondChangeIsDetected(t *testing.T) {
 ...
 if !localChanged(entry,baseline){t.Fatal("same-second edit was ignored")}
}
