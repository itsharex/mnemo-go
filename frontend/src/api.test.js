...
describe('api 纯逻辑辅助函数', () => {
  ...
  it('按文件类型选择打开方式，目录优先', () => {
    expect(openKindOf({ isDir: true, name: 'movie.mp4' })).toBe('dir')
		...
		expect(openKindOf({ name: 'movie.mkv' }, { cloudVideoPreview: true })).toBe('video')
		expect(openKindOf({ name: 'source.ts' }, { cloudVideoPreview: true })).toBe('text')
		expect(openKindOf({ name: 'stream.ts', mime_type: 'video/mp2t' }, { cloudVideoPreview: true })).toBe('video')
		...
		expect(openKindOf({ name: 'letter.docx' })).toBe('docx')
		expect(openKindOf({ name: 'budget.xlsx' })).toBe('xlsx')
		expect(openKindOf({ name: 'slides.pptx' })).toBe('pptx')
		expect(openKindOf({ name: 'legacy.doc' })).toBe('download')
		expect(openKindOf({ name: 'legacy.xls' })).toBe('download')
		expect(openKindOf({ name: 'legacy.ppt' })).toBe('download')
